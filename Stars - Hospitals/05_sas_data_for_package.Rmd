...
```{r}
df <- star_algorithm_input_list$`2018-01`
prov_or_score_columns <-
  colnames(df)[!grepl("(_DEN(_PRED|_VOL)?|_COMP|_RATE_P)$", colnames(df))]
melt(data.table(df[, prov_or_score_columns]), id.vars = "PROVIDER_ID",
     variable.name = "measure_id", value.name = "score")
unlist(lapply(df[, prov_or_score_columns], class))
```



```{r}
colnames(measure_info_df)
more_measures_info_dt %>%
  select(measure_id) %>%
  left_join(
    measure_info_df %>% select(cc_date, measure_id), by = "measure_id"
  ) %>%
  group_by(measure_id) %>%
  summarize(dates = sum(ifelse(is.na(cc_date), 0, 1))) %>%
  arrange(dates)
```


```{r}
star_algorithm_input_list$`2018-01` %>%
  pivot_longer(-PROVIDER_ID) %>%
  group_by(name) %>%
  summarize(
    is_na  = sum(ifelse(is.na(value), 1, 0)),
    not_na = sum(ifelse(is.na(value), 0, 1)),
    .groups = "drop"
  )
measures_202104_dt
```
