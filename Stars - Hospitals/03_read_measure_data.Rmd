...
care_compare_df <-
  ...
  # These two folders were zipped differently - e.g., the data is in
  # "zips/hospitals_10_2022/hospitals_10_2022" instead of "zips/hospitals_10_2022"
  filter(
    !(data_dir %in% c("zips/hospitals_10_2022", "zips/hospitals_01_2023"))
  ) %>%
  # The 2023-07 data was corrected in 2023-10
  filter(
    data_dir != "zips/hospitals_07_2023"
  ) %>%
  mutate(
    cc_month = case_when(
      data_dir == "zips/hospitals_10_2023" ~ 7,
      data_dir == "zips/hospitals_11_2023" ~ 10,
      TRUE ~ cc_month
    ),
    cc_date  = case_when(
      data_dir == "zips/hospitals_10_2023" ~ as.Date("2023-07-01"),
      data_dir == "zips/hospitals_11_2023" ~ as.Date("2023-10-01"),
      TRUE ~ cc_date
    )
  )
...
find_mort_readm_files <- function(data_dir) {
  if (grepl("_10_2020$", data_dir)) {
    ...
    x <- x[!grepl("^PCH_", x, ignore.case = TRUE)]
...
ipfqr_get_first_word <- function(x) {
  ...
  toupper(str_trim(str_split(x, "[_ ]")[[1]][1]))
}
ipfqr_get_first_word2 <- function(x) {
  str_match(str_trim(x), "^([^_ ]+)(?:[_ ][^_ ]+)*")[[2]]
}
...
read_ipfqr_data <- function(filename) {
  ...
  df1 <- temp_df %>% filter(!grepl("/", measure_id))
  df2 <- temp_df %>% filter(grepl("/", measure_id)) %>%
    mutate(
      measure_id = map(measure_id, ipfqr_split_slash_measure_id)
    ) %>%
    unnest(measure_id)
  #cat(str_c("\r\n\t\tdim(df1) = ", paste(dim(df1), collapse=", ")))
  #cat(str_c("\r\n\t\tdim(df2) = ", paste(dim(df2), collapse=", ")))
  df <-
    bind_rows(df1, df2) %>%
    transmute(
      provider_id,
...
# all_mort_readm_data_df <- all_mort_readm_data_df %>% filter(cc_id != 34)
# all_maternal_data_df <- all_maternal_data_df %>% filter(cc_id != 34)
# all_hai_data_df <- all_hai_data_df %>% filter(cc_id != 34)
# all_hcahps_data_df <- all_hcahps_data_df %>% filter(cc_id != 34)
# all_timely_data_df <- all_timely_data_df %>% filter(cc_id != 34)
# all_imaging_data_df <- all_imaging_data_df %>% filter(cc_id != 34)
# all_ipfqr_data_df <- all_ipfqr_data_df %>% filter(cc_id != 34)
```



The previous code chunk takes a long time, so save that data here

CAREFUL: These tables are saved at the end. May not want to overwrite them.

```{r}
save(all_mort_readm_data_df, file = file.path(dirs$rdata, "temp_all_mort_readm_data_df.RData"))
save(all_maternal_data_df,   file = file.path(dirs$rdata, "temp_all_maternal_data_df.RData"))
save(all_hai_data_df,        file = file.path(dirs$rdata, "temp_all_hai_data_df.RData"))
save(all_hcahps_data_df,     file = file.path(dirs$rdata, "temp_all_hcahps_data_df.RData"))
save(all_timely_data_df,     file = file.path(dirs$rdata, "temp_all_timely_data_df.RData"))
save(all_imaging_data_df,    file = file.path(dirs$rdata, "temp_all_imaging_data_df.RData"))
save(all_ipfqr_data_df,      file = file.path(dirs$rdata, "temp_all_ipfqr_data_df.RData"))
```


```{r}
load(file = file.path(dirs$rdata, "temp_all_mort_readm_data_df.RData"))
load(file = file.path(dirs$rdata, "temp_all_maternal_data_df.RData"))
load(file = file.path(dirs$rdata, "temp_all_hai_data_df.RData"))
load(file = file.path(dirs$rdata, "temp_all_hcahps_data_df.RData"))
load(file = file.path(dirs$rdata, "temp_all_timely_data_df.RData"))
load(file = file.path(dirs$rdata, "temp_all_imaging_data_df.RData"))
load(file = file.path(dirs$rdata, "temp_all_ipfqr_data_df.RData"))
```
...
```{r}
update_col_name_if_exists <- function(df, name_from, name_to, add_col = TRUE) {
...
update_col_names <- function(df, name_from_to_list, add_col = TRUE) {
  for (n in names(name_from_to_list)) {
    df <- update_col_name_if_exists(df, n, name_from_to_list[[n]], add_col)
  }
  df
}
  ...
  if (length(fname) > 0) {
    ...
    if (file.exists(fname)) {
      df <-
        ...
        mutate(across(everything(), str_trim))
      colnames(df)[1:2] <- c("provider_id", "hospital_name")
      city_cols <- grep("^city", colnames(df), ignore.case = TRUE)
      name_from_to_list <- list(
        "City/Town"        = "City",
        "County/Parish"    = "County Name",
        "Telephone Number" = "Phone Number"
      )
      df <- update_col_names(df, name_from_to_list, add_col = FALSE)
...
read_general_info_file <- function(zip_dir, zip_date) {
  ...
  other_name_updates <- list(
    "City/Town"                         = "City",
    "County/Parish"                     = "County Name",
    "Telephone Number"                  = "Phone Number"
  )
  df <- update_col_names(df, other_name_updates)
...
get_date_conversion_func <- function(str_vec) {
  str_vec <- str_vec[str_trim(str_vec) != ""]
...
read_measure_dates_file <- function(data_dir, cc_date) {
  ...
  if (length(fname) == 1) {
    ...
    if (file.exists(fname)) {
      ...
      df$`Start Date` <- gsub("^N/A$", "", df$`Start Date`)
      df$`End Date`   <- gsub("^N/A$", "", df$`End Date`)
...
read_general_info_file(care_compare_df$data_dir[1], care_compare_df$cc_date[1])
```
...
all_sas_input_df <-
  tribble(
    ...
    "alldata_2023jul.sas7bdat",  "2023-01-01"
  ) %>%
  mutate(
    sas_file_id = 1:n(),
    data = map(
...
# TODO: In the July 2022 CMS Care Compare Data Dictionary, there's a Release
#       Updates (Appendix F) note saying
#           "In July 2022, CMS will update the 2021 OP-10 measure
#            results to correct the calculation of error"
#       Does that mean the 2022-07 numbers should replace the OP-10 from
#       2021-07 through 2022-04?
# https://data.cms.gov/provider-data/archived-data/hospitals
...
# It's noted that the OP-13 scores for the year Jul '22 - Jun '23
# https://essentialhospitals.org/policy/july-2023-hospital-overall-star-ratings-preview-reports-available
replacement6 <- update_measures2(
  all_imaging_data_df, "Imaging", c("OP_13"),
  from_date = "2023-04-01", to_date = "2022-07-01"
)
replacement7 <- update_measures2(
  all_imaging_data_df, "Imaging", c("OP_13"),
  from_date = "2023-04-01", to_date = "2022-10-01"
)
replacement8 <- update_measures2(
  all_imaging_data_df, "Imaging", c("OP_13"),
  from_date = "2023-04-01", to_date = "2023-01-01"
)
all_imaging_data_df$data[[replacement6$index]] <- replacement6$df
all_imaging_data_df$data[[replacement7$index]] <- replacement7$df
all_imaging_data_df$data[[replacement8$index]] <- replacement8$df
...
# TODO: Update national rates for OP_13?
