...
initial_c_cut_point_processing <- function(...) {
  ...
  df <-
    row_df %$%
    read.csv(filename, skip = skip, nrows = nrows,
...
initial_d_cut_point_processing <- function(...) {
  ...
  df_colnames <- c("stars", measure_ids)
  #df_coltypes <- paste0(rep("c", length(df_colnames)), collapse="")
  max_cols <- dim(row_df %$% read_csv(filename, skip = skip+1))[2]
  print(max_cols)
  ...
  df <-
    suppressMessages(
      row_df %$%
      read_csv(filename, skip = skip+1, n_max = nrows, col_types = df_coltypes,
